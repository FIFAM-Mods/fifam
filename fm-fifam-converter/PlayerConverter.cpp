...
FifamPlayer *Converter::CreateAndConvertPlayer(UInt gameId, foom::player * p, FifamClub * club) {
    ...
    enum AppearanceSource {
        None, Generated, FIFA, Foom, Custom
    };
    AppearanceSource faceSource = AppearanceSource::None;
    AppearanceSource hairSource = AppearanceSource::None;
    AppearanceSource hairColorSource = AppearanceSource::None;
    ...
    auto NameFromMap = [](Int id, Map<Int, String> const &m) {
        if (Utils::Contains(m, id))
            return m.at(id);
        return String(L"n/a");
    };

    auto NameFromVector = [](Int index, Vector<String> const &v) {
        if (index < v.size())
            return v.at(index);
        return String(L"n/a");
    };

    static Map<Int, String> AppearanceSourceName = {
        { AppearanceSource::None, L"None" },
        { AppearanceSource::Generated, L"Generated" },
        { AppearanceSource::FIFA, L"FIFA" },
        { AppearanceSource::Foom, L"Foom" },
        { AppearanceSource::Custom, L"Custom" }
    };

    static Map<Int, String> AppearanceTypeName = {
        { 1, L"Western Europe Light" },
        { 2, L"Western Europe Dark" },
        { 3, L"African 1" },
        { 4, L"Arabic 1" },
        { 5, L"African 2" },
        { 6, L"Latin" },
        { 7, L"Asian" },
        { 8, L"Eastern Europe Light" },
        { 9, L"Eastern Europe Dark" },
        { 10, L"Arabic 2" },
        { 11, L"Indian" }
    };

    static Map<Int, String> FoomEthnicityName = {
        { -1, L"Unknown" },
        { 0, L"Northern European" },
        { 1, L"Mediterranean/Hispanic" },
        { 2, L"North African/Middle Eastern" },
        { 3, L"African/Caribbean" },
        { 4, L"Asian" },
        { 5, L"South East Asian" },
        { 6, L"Pacific Islander" },
        { 7, L"Native American" },
        { 8, L"Native Australian" },
        { 9, L"Mixed Race" },
        { 10, L"East Asian" }
    };

    static Map<Int, String> FoomHairLengthName = {
        { 0, L"Bald" },
        { 1, L"Short" },
        { 2, L"Medium" },
        { 3, L"Long" }
    ...
    static Map<Int, String> FoomHairColorName = {
        { 0, L"Black" },
        { 1, L"Blonde" },
        { 2, L"Brown" },
        { 3, L"Grey" },
    ...
    if (CountryMainAppearanceType.empty()) {
        ...
        for (UInt countryId = 1; countryId <= FifamDatabase::NUM_COUNTRIES; countryId++) {
            ...
            String countryName = country ? FifamTr(country->mName) : L"n/a";
            cw.WriteLineWithSeparator(L'\t', Quoted(countryName), NameFromMap(CountryMainAppearanceType[countryId], AppearanceTypeName));
        }
    }
    ...
    faceSource = hairSource = hairColorSource = AppearanceSource::Generated;
    ...
    if (p->mHairLength >= 0 && p->mHairLength <= 3) {
        ...
        if (hairStyle != -1) {
            ...
            hairSource = AppearanceSource::Foom;
        }
    }
    ...
    if (p->mHairColour >= 0 && p->mHairColour <= 6) {
        ...
        hairColorSource = AppearanceSource::Foom;
    ...
    if (p->mConverterData.mFifaPlayerId > 0) {
        ...
        faceSource = hairSource = hairColorSource = AppearanceSource::FIFA;
    }

    // fourth - apply custom appearance (if available)
    bool hasCustomFace = false;
    bool hasCustomHair = false;
    if (!p->mConverterData.mEditorFace.empty() && Utils::Contains(AppearanceGenerator::HeadIdFromEditorName, p->mConverterData.mEditorFace)) {
        player->mAppearance.mGenericFace = AppearanceGenerator::HeadIdFromEditorName[p->mConverterData.mEditorFace];
        hasCustomFace = true;
        faceSource = AppearanceSource::Custom;
    }
    if (p->mConverterData.mEditorHair != -1 && Utils::Contains(AppearanceGenerator::HairIdFromEditorId, (UInt)p->mConverterData.mEditorHair)) {
        player->mAppearance.mHairStyle = AppearanceGenerator::HairIdFromEditorId[p->mConverterData.mEditorHair];
        hasCustomHair = true;
        hairSource = AppearanceSource::Custom;
    }
    if (!p->mConverterData.mEditorBeard.empty()) {
        if (Utils::Contains(AppearanceGenerator::BeardIdFromEditorName, p->mConverterData.mEditorBeard))
            player->mAppearance.mBeardType = AppearanceGenerator::BeardIdFromEditorName[p->mConverterData.mEditorBeard];
        else if (hasCustomFace && hasCustomHair)
            player->mAppearance.mBeardType = FifamBeardType::None;
    }
    if (!p->mConverterData.mEditorEye.empty()) {
        if (Utils::Contains(AppearanceGenerator::EyeIdFromEditorName, p->mConverterData.mEditorEye))
            player->mAppearance.mEyeColour = AppearanceGenerator::EyeIdFromEditorName[p->mConverterData.mEditorEye];
        else if (hasCustomFace && hasCustomHair)
    ...
    if (p->mGender == 1)
        player->mAppearance.mBeardType = FifamBeardType::None;
    if (p->mWearsHijab) {
        player->mAppearance.mHairStyle = AppearanceGenerator::HairIdFromEditorId[1059];
        hairSource = AppearanceSource::Foom;
    }

    // Log
    if (mLogAppearance) {
        static FifamWriter appearanceLog("AppearanceLog.txt");
        static Bool headerWritten = false;
        if (!headerWritten) {
            appearanceLog.WriteLineWithSeparator(L'\t', L"ID", L"FifaID", L"Name", L"PlayerCountry", L"AppearanceCountry",
                L"Ethnicity", L"AppearanceGroup", L"CountryAppearanceGroup", L"HairLength", L"HairColor", L"FaceSource",
                L"HairSource", L"HairColorSource", L"ResultFace", L"ResultHair", L"ResultHairColor");
            headerWritten = true;
        }
        ...
        appearanceLog.WriteLineWithSeparator(L'\t', p->mID, player->mFifaID, Quoted(p->mFullName),
            ...
            NameFromMap(ethnicity, AppearanceTypeName),
            NameFromMap(countryEthnicity, AppearanceTypeName),
            NameFromMap(p->mHairLength, FoomHairLengthName),
            NameFromMap(p->mHairColour, FoomHairColorName),
            NameFromMap(faceSource, AppearanceSourceName),
            NameFromMap(hairSource, AppearanceSourceName),
            NameFromMap(hairColorSource, AppearanceSourceName),
            NameFromVector(player->mAppearance.mGenericFace, AppearanceGenerator::FaceNames),
            NameFromVector(player->mAppearance.mHairStyle, AppearanceGenerator::HairNames),
            NameFromVector(player->mAppearance.mHairColor, AppearanceGenerator::HairColorNames));
    }
