...
void resizeImage_noAspect(Image &image, int w, int h) {
    Geometry geom(w, h);
    geom.aspect(true);
    image.resize(geom);
}

bool ConvertKit(string const &inputShirt, string const &inputShorts, string const &inputCrest, string const &outputFile) {
    if (options.OutputGameId >= 9) {
        ...
        outputFileImg.fillColor(Color(inputShirtImg.pixelColor(478, 518)));
        ...
        resizeImage_noAspect(shirtMainImg, 350, 256);
        outputFileImg.composite(shirtMainImg, 162, 0, OverCompositeOp);

        Image shirtBackImg(inputShirtImg, Geometry(377, 480, 323, 68));
        resizeImage_noAspect(shirtBackImg, 350, 256);
        shirtBackImg.flip();
        ...
        outputFileImg.composite(rightSockImg2, 81, 2 + 71, OverCompositeOp);
        ...
        if (!inputCrest.empty()) {
            Image inputCrestImg(inputCrest);
            resizeImage_noAspect(inputCrestImg, 34, 34);
            outputFileImg.composite(inputCrestImg, 382, 54, OverCompositeOp);
        }
...
bool ConvertFifaClubKit(int fifaId, int set, int variation, string const &outputFile) {
...
int main(int argc, char **argv) {
    ...
    options.OutputGameId = 8;
    options.ConvertGkKit = false;
    options.ConvertThirdKit = false;

    ConvertClubKits(1,  0x000E0001);
    ConvertClubKits(5,  0x000E0009);
    ConvertClubKits(9,  0x000E0005);
    ConvertClubKits(10, 0x000E000E);
    ConvertClubKits(11, 0x000E000F);

    ConvertClubKits(1881, 0x00533007);
